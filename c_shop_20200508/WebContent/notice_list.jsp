...
<body>
	...
	<section>
		...
		<table>
			...
			<!-- 내용부분 -->
			<c:forEach var="list" items="${nbdtos }">
				<tr>
					<td><span class="table-notice">${list.getbId() }</span></td>
					<td>
						<c:forEach varStatus="vs" begin="1" end="${list.getbIndent() }">&nbsp&nbsp</c:forEach>
						<c:if test="${list.getbIndent() > 0 }">┖</c:if>
						<a href="notice_content_view.do?bId=${list.getbId() }">${list.getbTitle() }</a>
					</td>
					<td>${list.getbName() }</td>
					<td>${list.getbDate() }</td>
					<td>${list.getbHit() }</td>				
		...
		<c:choose>
			<c:when test="${searchflag != null }">
				<ul class="page-num">
					...
					<c:if test="${page > 1 }">
						<a href="notice_list.do?searchflag=${searchflag }&opt=${opt }&search=${search }&page=${page-1 }"><li class="prev"></li></a>
					</c:if>
					...
					<c:forEach var="a" begin="${startpage }" end="${endpage }" step="1">
						<c:choose>
							...
							<c:when test="${a != page }">
								<a href="notice_list.do?searchflag=${searchflag }&opt=${opt }&search=${search }&page=${a }">
									<li class="num"><div>${a }</div></li>
								</a>
							</c:when>
						</c:choose>
					...
					<c:if test="${page < maxpage }">
						<a href="notice_list.do?searchflag=${searchflag }&opt=${opt }&search=${search }&page=${page+1 }"><li class="next"></li></a>
					</c:if>
			...
			<c:otherwise>
				<ul class="page-num">
					...
					<c:if test="${page > 1 }">
						<a href="notice_list.do?page=${page-1 }"><li class="prev"></li></a>
					</c:if>
					...
					<c:if test="${page < maxpage }">
						<a href="notice_list.do?page=${page+1 }"><li class="next"></li></a>
					</c:if>
