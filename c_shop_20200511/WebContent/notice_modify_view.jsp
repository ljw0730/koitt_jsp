...
<body>
	...
	<section>
		...
		<form action="notice_modify.do" name="modify" method="post">
			<table>
			<input type="hidden" name="bId" value="${modify_view.getbId() }" />
			<input type="hidden" name="bGroup" value="${modify_view.getbGroup() }" />
			<input type="hidden" name="bStep" value="${modify_view.getbStep() }" />
			<input type="hidden" name="bIndent" value="${modify_view.getbIndent() }" />			
				<colgroup>
					<col width="15%">
					<col width="85%">
				</colgroup>
				<tr>
					<th>작성자</th>
					<td><input type="text" name="bName" value="${modify_view.getbName() }" readonly></td>
				</tr>
				<tr>
					<th>제목</th>
					<td><input type="text" name="bTitle" value="${modify_view.getbTitle() }"></td>
				</tr>
				<tr>
					<th>내용</th>
					<td>
						<textarea name="bContent" cols="50" rows="10">
						${modify_view.getbContent() }
						</textarea>
					</td>
				</tr>
				<tr>
			...
			<div class="button-wrapper">
				<button type="button" class="write" onclick="modify_check()">수정완료</button>
				<button type="reset" class="cancel" onclick="javascript:window.location.href='notice_list.do'">취소</button>
			</div>
		</form>
